...
#预处理数据集
#BaseDataSets 类继承自 torch.utils.data.Dataset，是一个自定义的数据集类，用于从HDF5文件中加载数据。
class BaseDataSets(Dataset):
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        #base_dir：数据集所在基本目录路径
        #split：数据集划分方式，train/val
        #num：数据集划分后，数据集的数量
        ...
        #初始化数据样本列表
        #如果是训练集，则从train_slices.list中读取样本列表；如果是验证集，则从val.list中读取样本列表
        if self.split == 'train':
