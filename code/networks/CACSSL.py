...
#该函数接受一个张量x作为输入。
#使用softmax函数对输入张量的第1维进行归一化处理，并将结果乘以-1。
#使用max_pool3d函数对处理后的张量进行最大池化操作，得到min_x（实际上是找到局部最小值）。
#对min_x再次进行最大池化操作，得到max_x（实际上是找到局部最大值）。
#计算max_x与min_x的差值，然后使用relu函数进行非线性变换，并在第1维上求和，最后在第0维上增加一个维度，形成掩码mask。
#使用F.interpolate函数将mask放大2倍，插值模式为trilinear。
#返回这个放大后的掩码mask。
#这个函数可能用于在3D图像中突出或提取轮廓信息。
def contour_attention_func(x):
    ...
    return mask
#这是一个继承自nn.Module的卷积块类。
#在初始化函数__init__中，根据传入的参数n_stages、n_filters_in、n_filters_out和normalization来构建卷积层和归一化层。
#在前向传播函数forward中，将输入张量x通过构建好的卷积层和归一化层，最后返回处理后的张量
...
class Decoder(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
                 has_residual=False, up_type=0, emb_num=16):
...
class Decoder_Contour(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
                 has_residual=False, up_type=0, attention_func=None, emb_num=16):
...
class CACNet3d(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
        ...
        self.decoder = Decoder(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual, 0,
                               emb_num=emb_num)
        self.decoder_contour = Decoder_Contour(n_channels, n_classes, n_filters, normalization, has_dropout,
                                               has_residual, 0, contour_attention_func, emb_num=emb_num)

    def forward(self, input):
        features = self.encoder(input)
        out_seg1, out_seg2, out_seg3, out_seg4, out_seg5, proj_out1 = self.decoder(features)
        out_contour_seg1, out_contour_seg2, out_contour_seg3, out_contour_seg4, out_contour_seg5, proj_contour_out1 = self.decoder_contour(
            features)
...
if __name__ == '__main__':
    ...
    model = CACNet3d(n_channels=1, n_classes=2, normalization='batchnorm', has_dropout=False)
