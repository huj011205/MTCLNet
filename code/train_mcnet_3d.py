...
parser.add_argument('--exp', type=str, default='MCNet-8', help='exp_name')
...
if __name__ == "__main__":
    ...
    batch_sampler = TwoStreamBatchSampler(labeled_idxs, unlabeled_idxs, args.batch_size, args.batch_size - labeled_bs)  # 样本采样器
    ...
    # optimizer = optim.SGD(model.encoder.parameters(), lr=base_lr, momentum=0.9, weight_decay=0.0001)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            y_ori = torch.zeros((num_outputs,) + outputs[0].shape)  # 存储模型输出的概率分布
            ...
            if iter_num >= 800 and iter_num % 200 == 0:
                ...
                snapshot_img = torch.zeros(
                    ...
                    dtype=torch.float32)  # 创建一个全0的5维张量 用于存储可视化结果
